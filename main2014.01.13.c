    ...
    */
     ...
     // for(;fgets(streamSearchF5buf,1024,streamSearchF5) != NULL;)
      if(fgets(streamSearchF5buf,1024,streamSearchF5) != NULL)//只取出一条分析
		...
		strcpy(streamSearchF5bufTemp,streamSearchF5buf);
		...
		{	
            ...
            if(res_ptr) {               
              ...
              //  while((sqlrow = mysql_fetch_row(res_ptr)))  {   //依次取出记录  
				{
					sqlrow=mysql_fetch_row(res_ptr);//只读一行就可以了
                   // for(i = 0; i < jj; i++)  
						//处理得到的sql语句，保留where前面的，若没有where全部输出
						GetSqlBeforeWhere(sqlrow[0]);
#ifdef DEBUG       
                    printf("sql:%s\t", sqlrow[0]);  //输出 
                    printf("\n"); 
					sleep(1);    
#endif           
					////////////写入redis
        reply = redisCommand(c,"lpush iplist%s:%s %s %s",RequestLogNewName,buff[1],streamSearchF5buf,sqlrow[0]);
        printf("lpush iplist%s:%s %s %s\n",RequestLogNewName,buff[1],streamSearchF5buf,sqlrow[0]);
		printf("lpush: %s\n", reply->str);
