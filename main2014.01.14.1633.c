					...
					{        
						...
						{      
							...
							if(mysql_num_rows(res_ptr)==0)
							{
								...
								printf("lpush iplist%s %s\n",RequestLogNewName,streamSearchF5buf);
								#endif
								////////////写入redis
								reply = redisCommand(c,"lpush iplist%s %s",RequestLogNewName,streamSearchF5buf);	
								#ifdef DEBUG  
								printf("lpush: %s\n", reply->str);
								#endif
							} 
							else
							{
								...
								sprintf(InsertToList,"%s %s",streamSearchF5buf,sqlrow[0]);
								////////////写入redis
